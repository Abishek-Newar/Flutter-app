...
import 'package:naqde_user/features/transaction_money/domain/models/hex_color_model.dart';
...
class _PurposeItem extends StatelessWidget {
  const _PurposeItem({required this.image, required this.title, required this.color,required this.onTap});
  final String? image;
  final String? title;
  final Color color;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(4.0),
          color: Colors.white,
          boxShadow: [BoxShadow(blurRadius: 20.0,color: Colors.black.withValues(alpha:0.05),spreadRadius: 0.0,offset: const Offset(0.0, 4.0)),]
      ),
        ...
        child: Column(
          children: [
            ...
            Expanded(
              flex: 3,
              child: Center(
                child: Text(title!,textAlign: TextAlign.center,style: rubikRegular.copyWith(fontSize: Dimensions.fontSizeDefault,color: Theme.of(context).hintColor),),
              ),
            )
