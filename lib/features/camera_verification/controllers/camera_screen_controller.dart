...
class CameraScreenController extends GetxController implements GetxService{
  ...
  void valueInitialize(bool fromEditProfile) {
    _eyeBlink = 0;
    _isSuccess = 0;
    _fromEditProfile = fromEditProfile;
  }

  Future startLiveFeed({bool isQrCodeScan = false,bool isHome = false, String? transactionType = '', bool fromSearchContact = false}) async {
    ...
    controller?.initialize().then((_) {
      ...
      controller?.startImageStream((CameraImage cameraImage) => _inputImageFromCameraImage(
        ...
        isQrCodeScan: isQrCodeScan, transactionType: transactionType, fromSearchContact: fromSearchContact
      ));
  ...
  Future<void> _inputImageFromCameraImage({
    ...
    if(isQrCodeScan) {
      Get.find<QrCodeScannerController>().processImage(inputImage, isHome, transactionType, fromSearchContact: fromSearchContact, callBack: () async {
       await controller?.stopImageStream();
      });
    }else{
    ...
    if(_eyeBlink == 3) {
      try{
        await controller?.stopImageStream().then((value)async {
          ...
          XFile compressXFile = await compressFile(File(file.path));
