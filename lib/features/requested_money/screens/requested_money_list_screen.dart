...
import 'package:naqde_user/features/requested_money/widgets/requested_money_screen.dart';
...
class RequestedMoneyListScreen extends StatefulWidget {
  ...
  const RequestedMoneyListScreen({super.key, required this.requestType, this.isFromNotification});
...
class _RequestedMoneyListScreenState extends State<RequestedMoneyListScreen> {
  ...
  Future<void> loadData(bool formReload) async {
    ...
    return TextButton(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            color: Get.find<RequestedMoneyController>().requestTypeIndex == index ? Theme.of(context).colorScheme.secondary :  Theme.of(context).cardColor,
        ...
        child: Padding(
          ...
          child: Text('$text($length)',
              style: rubikSemiBold.copyWith(color: Get.find<RequestedMoneyController>().requestTypeIndex == index
                  ? Colors.black : Theme.of(context).textTheme.titleLarge!.color)),
        ),
      ),
    );
  }
